// MainWindow.cpp
// Requiere Qt 6 (o Qt 5 con ajustes mínimos).
// El motor corre en hilos de trabajo; la UI se actualiza vía Qt signals
// emitidos desde los callbacks del EventBus (thread-safe por Qt::QueuedConnection).
...
// ─────────────────────────────────────────────────────────────────────────────
// CopyWorker: ejecuta el escaneo + copia en un hilo separado de Qt
// ─────────────────────────────────────────────────────────────────────────────
...
public slots:
    void run() {
        ...
        pool.WaitAll();
        bus.Emit(EventQueueFinished{
            entries.size(),
            static_cast<size_t>(totalOk.load()),
            static_cast<size_t>(totalFail.load())
        });

        emit finished(static_cast<int>(entries.size()),
                      totalOk.load(), totalFail.load());
    }

    void requestPause()  { /* implementar vía flag compartido */ }
    void requestCancel() { /* implementar vía flag compartido */ }
...
private slots:
    ...
    void OnStart() {
        ...
        connect(worker, &CopyWorker::finished, this, [this, worker, thread]
                (int total, int ok, int failed) {
            statusBar()->showMessage(
                QString("Done: %1 ok, %2 failed of %3 total").arg(ok).arg(failed).arg(total));
    ...
    void OnPause()  { /* señal al worker */ }
    void OnCancel() { /* señal al worker */ }

    // Slots actualizados desde EventBus (hilo seguro vía invokeMethod)
...
private:
    ...
    // Conecta EventBus → UI de forma thread-safe
